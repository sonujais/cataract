    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect('history')  # Redirect to the image history
...
# Function for doctors to respond to a recommendation request
@login_required
...
def recommendation_history(request):
    ...
    recommendations = RecommendationRequest.objects.filter(doctor=doctor)
...
def edit_recommendation(request, recommendation_id):
    ...
    if request.method == 'POST':
        ...
        recommendation.comments = comments  # Update the comments
        recommendation.save()  # Save the changes
